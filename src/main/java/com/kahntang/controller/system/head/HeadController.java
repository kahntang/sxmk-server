package com.kahntang.controller.system.head;
...
import com.kahntang.service.system.appuser.AppuserService;
...
import com.kahntang.util.SmsUtil;
import com.kahntang.util.Tools;
import com.kahntang.util.Watermark;
import com.kahntang.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",sxmk,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",sxmk,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",sxmk,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",sxmk,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",sxmk,");
			if(strIW.length == 4){
		...
		if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
			String strIW[] = strWEBSOCKET.split(",sxmk,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",sxmk,"+pd.getString("PORT")+",sxmk,"+pd.getString("EMAIL")+",sxmk,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",sxmk,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",sxmk,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",sxmk,"+pd.getString("fcontent")+",sxmk,"+pd.getString("fontSize")+",sxmk,"+pd.getString("fontX")+",sxmk,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",sxmk,"+pd.getString("imgUrl")+",sxmk,"+pd.getString("imgX")+",sxmk,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",sxmk,"+pd.getString("WIMPORT")+",sxmk,"+pd.getString("OLIP")+",sxmk,"+pd.getString("OLPORT"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
